 ...
 */
 ...
 //Colors 
 //Storing the sum of the X, Y and Z values
 float sumX = 0;
 float sumY = 0;
 float sumZ = 0;
  
 //number of active points for colour 
 int    countActivePoints = 0;
...
void handState(int handState) {
  ...
  * Colour Interface
  *=========================*/
  void updateColourState(){
      for (ArrayList<SoundPoint> points : strokes) {
          //Find the point with the min dist in the stroke and play
          sumMinCoordinate(points);
      }  
      
      if(countActivePoints != 0){
          float xval = sumX/countActivePoints;
          float yval = sumY/countActivePoints;
          float zval = sumZ/countActivePoints;
          int   xscaled = round(map(xval,-1,1,0,255));
          int   yscaled = round(map(yval,-1,1,0,255));
          int   zscaled = round(map(zval,0,2.5,0,255));
      
          //Write to serial port
          myPort.write(char(xscaled));         
          myPort.write(char(yscaled));
          myPort.write(char(zscaled));
  ...
  void sumMinCoordinate(ArrayList<SoundPoint> points){
      float minDist = Integer.MAX_VALUE;
      SoundPoint point = null;
      ...
      if(point != NULL){
          int xval = round(point.position.x);
          int yval = round(point.position.y);
          int zval = round(point.position.z;
          sumX = sumX + xval;
          sumY = sumY + yval;
          sumZ = sumZ + zval;
          countActivePoints = countActivePoints +1;
      }
