 ...
 */
 ...
 class SoundPoint {
   ...
   SoundPoint(PVector position, int analogReading) {
     ...
     this.frequency = analogReadingToFrequency(analogReading);
...
void handState(int handState) {
  ...
  *=========================*/
  ...
  void playStroke(Stroke stroke) {
    float minDist = Integer.MAX_VALUE;
      SoundPoint closestPoint = null;

      for(SoundPoint sp : stroke.points) {
      ...
      if(closestPoint == null) { println("CLOSEST POINT IS NULL"); return; }
      
      stroke.wp.setFrequency(closestPoint.frequency);
      stroke.glide.setValue(getAmplitude(minDist));
